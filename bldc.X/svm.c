...
void SVM(int16_t volts, uint16_t angle, MCAPP_DUTYCYCLEOUT_T *pdc)
{
	...
	if(angle < VECTOR4)
	{
		if(angle < VECTOR3)
		{
			if(angle < VECTOR2)
			{
				...
				voltstpwm = (int)(__builtin_mulss(volts,tpwm) >> 15);
				t1 = (int)(__builtin_mulss(t1,voltstpwm) >> 15);
				t2 = (int)(__builtin_mulss(t2,voltstpwm) >> 15);
    ...
    /* Duty Interchange  - dutycycle1 to PDC3 & dutycycle3 to PDC1  
       -In order to get standard wire connection (color code) at motor terminal connector*/
    pdc->dutycycle1 = PDC3Latch;    
    pdc->dutycycle2 = PDC2Latch;
    pdc->dutycycle3 = PDC1Latch;
